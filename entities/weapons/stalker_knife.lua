...
SWEP.Gun = ("stalker_knife") -- must be the name of your swep but NO CAPITALS!
SWEP.Category				= "S.T.A.L.K.E.R"
SWEP.Author				= ""
SWEP.Contact				= ""
SWEP.Purpose				= ""
SWEP.Instructions				= ("Left click to slash")
SWEP.PrintName				= "Trench knife"		-- Weapon name (Shown on HUD)	
...
SWEP.ViewModel				= "models/weapons/v_knife_stalker.mdl"	-- Weapon view model
SWEP.WorldModel				= "models/weapons/w_knife_stalker.mdl"	-- Weapon world model
...
SWEP.KnifeSlash = Sound("weapons/blades/slash.wav")
...
function SWEP:Deploy()
	...
	if file.Exists("weapons/knife/knife_deploy1.wav", "GAME") then
		...
		if self.Owner:Alive() then if self.Owner:GetActiveWeapon():GetClass() == self.Gun then
			...
			if slashtrace.Hit then
				...
				if targ:IsPlayer() or targ:IsNPC() then
					...
					paininfo:SetDamageForce(slashtrace.Normal *500)
