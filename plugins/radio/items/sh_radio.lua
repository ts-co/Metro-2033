...
function ITEM:getDesc()	
	if (!self.entity or !IsValid(self.entity)) then
		return L("radioDesc", getText(self:getData("power")), self:getData("freq", "000.0"))
	else
		local data = self.entity:getData()
		
		return L("radioDescEnt", (self.entity:getData("power") and "On" or "Off"), self.entity:getData("freq", "000.0"))
	end
end
