local PANEL = {}
	function PANEL:Init()
		...
		if (!suppress or (suppress and !suppress.all)) then
			...
			if (!suppress or !suppress.info) then
				...
				self.info:DockMargin(0, ScrH() * 0.5, ScrW() * 0.05, 0)
			end
	...
	function PANEL:setup()
		...
		if (self.name) then
			local class = nut.class.list[LocalPlayer():getChar():getClass()]
			local classColor = class.color or team.GetColor(LocalPlayer():Team())
			self.name:SetTextColor(classColor)
